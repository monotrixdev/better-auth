      ...
      {!session?.user && (
        <div className="flex space-x-2 items-center">
          <Image
            width={30}
            height={30}
            alt="Logo Monotrixdev"
            src="/logo.png"
          />
          <h4 className="text-sm font-semibold">Monotrix<span className="text-red-600">DEV</span></h4>
        </div>
      )}
      {session?.user && (
        <PanelLeft
        ...
        <div className="flex items-center justify-end">
            ...
            <>
              <div className="space-x-2 w-full flex items-center">
                ...
                <div className="p-2 relative rounded-full bg-indigo-600/10 hover:bg-purple-600/20">
                  <div className="absolute w-4 h-4 text-center top-1 right-0 rounded-full bg-purple-700 text-xs text-white">
                    2
                  </div>
          ...
          {!session?.user && (
            <>
              ...
              <Link
                href="/login"
                className="group ml-2 p-[1px] bg-gradient-to-br from-purple-800 via-purple-500 to-purple-600 rounded-full"
              >
...
export const ItemsShow = ({
  ...
  return (
    <Link href={href}>
      <Button
        variant="link"
        className={`${active ? "bg-gradient-to-br from-zinc-400 via-zinc-500 to-zinc-700" : ""}`}
      >
        {label}
      </Button>
    </Link>
